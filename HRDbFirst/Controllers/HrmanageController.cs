...
namespace HRDbFirst.Controllers
{
    public class HrmanageController : Controller
    {
        ...
        {
            string[] leavInts = { "6", "8", "10" };
            ViewData["LeaveApplication"] = _context.tbl_LeaveDefinition.Where(x => x.LeaveId == 6 || x.LeaveId == 8 || x.LeaveId == 10).Select(x => new { Name = x.LeaveName, Id = x.LeaveCode }).ToList();
            return View();
        }
        [HttpPost]
